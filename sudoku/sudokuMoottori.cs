...
namespace sudoku
{
    public class sudokuMoottori
    {
        ...
        #region Luo Peli v0.1
        public void LuoPeliV01()
        {
            int maxYrityksia = 10000;
            ...
            do
            {
                ...
                Pakka[] rivit = new Pakka[9]; // Tehdään pelialueen riveistä ja sarakkeista "pakat", jotka sisältää siis numerot 1-9
                Pakka[] sarakkeet = new Pakka[9];
                ...
                Pakka aliruudukonNumerot = new Pakka(rnd); // tehdään myös 3x3 ruudukosta "pakka", joka sisältää numerot 1-9
                aliruudukonNumerot.Alusta();


                for (int tr = 0; tr < 3; tr++) // pelialueen 3x3 ruudukon rivitieto
                {
                    for (int tc = 0; tc < 3; tc++) // pelialueen 3x3 ruudukon saraketieto
                    {
                        aliruudukonNumerot.Alusta(); // alustetaan 3x3 ruudukon pakka
                        for (int r = 0; r < 3; r++) // kyseisen 3x3 ruudukon rivitieto
                        {
                            for (int s = 0; s < 3; s++) // kyseisen 3x3 ruudukon saraketieto
                            {
                                ...
                                do
                                {
                                    int ehdotettuNumero = aliruudukonNumerot.NostaJaPoistaSatunnainen(); // arvotaan 3x3 pakasta joku numero

                                    samaNumero = false; // lähdetään siitä, että ei ole sama numero kuin jo arvottu

                                    if (!(rivit[r + (tr * 3)].onkoPakassa(ehdotettuNumero))) // tsekataan onko numero pelattu jo kyseisellä rivillä
                                    {
                                        samaNumero = true; // ja kun numeroa ei löydy pakasta, niin tarkoittaa sitä että se on jo pelattu
                                    }
                                    else if (!(sarakkeet[s + (tc * 3)].onkoPakassa(ehdotettuNumero))) // ja sama tsekkaus sarakkeelle
                                    {
                                        samaNumero = true;
                                    }


                                    if (samaNumero == false) // mikäli kaikki ehdot täyttyi, niin pelataan arvottu numero pelilaudalle
                                    {
                                        peliruudukko[r + (tr * 3), s + (tc * 3)] = ehdotettuNumero;
                                        rivit[r + (tr * 3)].PoistaNumero(ehdotettuNumero); // poistetaan pelattu numero kyseiseltä riviltä
                                        sarakkeet[s + (tc * 3)].PoistaNumero(ehdotettuNumero); // ja sarakkeelta
                                    }
                                    else
                                    {
                                        aliruudukonNumerot.Lisaa(ehdotettuNumero); // Numero ei käy, joten lisätään se takaisin pakkaan
                                    }

                                } while (samaNumero && yrityksia++ < maxYrityksia); // loopataan niin pitkään että joko yritykset tulee täyteen, tai samaNumero on false

                                if (yrityksia >= maxYrityksia) // luovutaan kyseisen taulukon arvonnasta, kun yritykset tullut täyteen
                                    arvotaan = true; // vaihdetaan arvotaan trueksi, jotta arvonta alkaa alusta
                            }
                        }
                    }
                }
